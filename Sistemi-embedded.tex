...
\begin{itemize}
    ...
    \item{CLOSE,} è lo stato che si raggiunge quando la seconda fotocellula è occupata, tutti i led sono spenti. Quando il sistema di trova in questo stato è possibile impostare il tempo di lavoro e quello di chiusura.
    \item{OPENING,} è lo stato di apertura durante il quale in led giallo lampeggia.
    \item{OPEN,} è lo stato in cui il cancello è aperto e tutti i led sono accesi.
\end{itemize}

In parallelo a questi stati ci sono gli stati \textbf{B1}, \textbf{B2} e \textbf{B3}, che "ascoltano" i fronti di salita provenienti dai rispettivi bottoni.
...
\begin{figure}[h] % 'h' significa "here", posiziona l'immagine qui
    \centering % Centra l'immagine
    \includegraphics[width=0.9\textwidth]{state_diagram0.png} % Inserisce l'immagine con una larghezza del 50% del testo
    \caption{Use case diagram} % Aggiunge una didascalia
    \label{fig:immagine} % Aggiunge un'etichetta per riferimenti incrociati
\end{figure}
%\newpage
